defmodule ChatUtils do
  ...
  ip_cadena = case Enum.filter(direcciones, fn {ip, _, _} ->
    ...
    # Excluir loopback y IPs de enlace local
    ...
    _ ->
      # Si no encuentra una IP adecuada, usar direcciones específicas o localhost
      case :os.type() do
        {:unix, :linux} ->
          {result, output} = System.cmd("hostname", ["-I"])
          String.trim(output) |> String.split() |> List.first() || "localhost"
        _ ->
          "localhost"  # Fallback
      end
  end
